...
export const createCloverCoinAppServer = async (options: ServerOptions) => {
  ...
  /**
   * "New" module system entry point.
   * It's a lowkey TODO to move all of these register calls below into
   * modules.
   */
  const app = AppModule.instantiate().initialize();
  const rootContainer = app.container;
